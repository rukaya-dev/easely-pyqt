...
from views.componenets.trash.context_menu import ContextMenu

logger = set_up_logger('views.report.trash_management_view.table.table_layout')


class TableLayout(QWidget):
    viewSignal = pyqtSignal()
    revertSignal = pyqtSignal()
    deleteSignal = pyqtSignal()

    def __init__(self, data_controller, data_columns=None, column_display_names=None, is_pagination=True,
                 parent=None):
        ...
        self.table_model = TableModel(table_name="patients_trash", data_columns=data_columns,
                                      column_display_names=column_display_names)

        # Palette
        palette = QApplication.instance().palette()
        self.main_bg_color_on_hover = palette.color(QPalette.ColorRole.Highlight).name()
        self.text_color = palette.color(QPalette.ColorRole.Button).name()

        self.btn_text_color = palette.color(QPalette.ColorRole.ButtonText).name()
        self.bg_color = palette.color(QPalette.ColorRole.Base).name()
    ...
    async def nextPage(self):
        ...
        try:
            ...
            await self.controller.get_items(page_number=self.parent.data["next_page"], item_per_page=10)
    ...
    async def prevPage(self):
        ...
        try:
            await self.controller.get_items(page_number=self.parent.data["prev_page"], item_per_page=10)
    ...
    def show_context_menu(self, position):
        if position is not None:
            ...
            self.contextMenu.itemSelected.connect(
                lambda item_id, row=currentRow: self.menu_item_triggered(item_id, row))
