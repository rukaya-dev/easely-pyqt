...
class FilterWidget(QWidget):
    def __init__(self, categories, template_controller, parent=None):
        ...
        # API model
        self.categories_controller = CategoryController()
        ...
        self.scroll_area.setMinimumSize(500, 380)
    ...
    def populate_categories(self):
        # categories = await self.categories_controller.get_all_categories()
        if self.categories:
    ...
    async def clear_filter(self):
        self.signals.globalCreateLoadingNotificationSignal.emit("CLEAR_TEMPLATE_FILTER")
        ...
        self.template_controller.store.set_filter_preferences({})
        
        await self.parent.refresh_table()
        self.signals.globalLoadingNotificationControllerSignal.emit("CLEAR_TEMPLATE_FILTER")
        self.close_menu()

    @pyqtSlot()
    @asyncSlot()
    async def apply_filter(self):
        self.signals.globalCreateLoadingNotificationSignal.emit("APPLY_TEMPLATE_FILTER")
        ...
        self.template_controller.store.set_filter_preferences(preferences)

        await self.parent.refresh_table()
        self.signals.globalLoadingNotificationControllerSignal.emit("APPLY_TEMPLATE_FILTER")
        self.close_menu()
