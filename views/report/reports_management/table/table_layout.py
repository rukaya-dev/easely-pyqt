...
from views.report.reports_management.table.context_menu import ContextMenu

logger = set_up_logger('views.reports.table.table_layout')


class TableLayout(QWidget):
    viewSignal = pyqtSignal()
    editStatusSignal = pyqtSignal()
    editReportContentSignal = pyqtSignal()
    printReportSignal = pyqtSignal()
    deleteReportSignal = pyqtSignal()
    ...
    def __init__(self, data_controller, data_columns=None, column_display_names=None, parent=None):
        ...
        self.signals.globalReportUpdateTableViewSignal.connect(self.update_table_view)

        self.table_model = TableModel(table_name="reports", data_columns=data_columns,
    ...
    def update_table_view(self):
        ...
        tab_data_map = {
            ...
            "drafted": self.controller.store.get_drafted_tab_reports(),
            "recent": self.controller.store.get_recent_tab_reports(),
            "archived": self.controller.store.get_archived_tab_reports(),
            "finalized": self.controller.store.get_finalized_tab_reports(),
    ...
    async def nextPage(self):
        ...
        try:
            ...
            if self.active_tab == "search_filter":
                await self.controller.get_items(item_per_page=20, page_number=next_page)
            else:
                await self.controller.get_items_by_tabs(self.active_tab, item_per_page=20, page_number=next_page)
    ...
    async def prevPage(self):
        ...
        try:
            ...
            if self.active_tab == "search_filter":
                await self.controller.get_items(item_per_page=20, page_number=prev_page)
            else:
                await self.controller.get_items_by_tabs(self.active_tab, item_per_page=20, page_number=prev_page)
    ...
    def menu_item_triggered(self, item_id, current_row_index):
        ...
        if item_id == "view":
            ...
            self.printReportSignal.emit()

        elif item_id == "edit_report_content":
            self.editReportContentSignal.emit()

        elif item_id == "delete":
            self.deleteReportSignal.emit()
