...
logger = set_up_logger('main.views.report.trash_management.trash_management_view')
...
class TrashManagementView(QWidget):
    def __init__(self):
        ...
        self.signals.updateReportsTrashTableSignal.connect(self.refresh_table)

        # API model
        self.trash_controller = ReportTrashController()
    ...
    def setup_table_ui(self):
        ...
        self.filter_widget = FilterWidget(model_name="reports_trash", controller=self.trash_controller,
                                          menu_pos="right", parent=self)
    ...
    async def view_trash(self):
        if self.table_view.get_current_id():
            ...
            if not data:
                ...
                data_as_html = convert_to_html_text(json.loads(data["report_info"]))
        ...
        if self.table_view.get_current_id():
            ...
            if not data:
                ...
                if not res:
                    ...
                    self.signals.refreshReportsAllTabSignal.emit()
                    await self.refresh_table()
