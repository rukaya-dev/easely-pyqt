...
from views.settings.users.table.context_menu import ContextMenu

logger = set_up_logger('views.settings.users.table.table_layout')


class TableLayout(QWidget):
    viewSignal = pyqtSignal(str)
    editPasswordSignal = pyqtSignal()
    ...
    def __init__(self, table_name, data_controller, data_columns=None, column_display_names=None, is_pagination=True,
        ...
        self.internal_loader = InternalLoader(height=40, parent=self)
        # self.internal_loader.start()

        self.table_model = TableModel(table_name=self.table_name, data_columns=data_columns,
        ...
        # self.pagination_widget.setStyleSheet("background-color:red;")
    ...
    def menu_item_triggered(self, item_id, current_row_index):
        ...
        elif item_id == "edit_user_info":
            self.viewSignal.emit("edit_user_info")

        elif item_id == "edit_password":
            self.editPasswordSignal.emit()
