...
from views.billing.table.billing_context_menu import BillingContextMenu
...
logger = set_up_logger('main.views.staff.billings.table.table_layout')


class BillingTableLayout(QWidget):
    viewSignal = pyqtSignal(str)
    ...
    def __init__(self, data_controller, data_columns=None, column_display_names=None, is_pagination=True,
        ...
        self.table_model = TableModel(table_name="doctors", data_columns=data_columns,
    ...
    def show_context_menu(self, position):
        if position is not None:
            ...
            self.contextMenu = BillingContextMenu()
    ...
    def menu_item_triggered(self, item_id, current_row_index):
        ...
        if item_id == "view":
            self.viewSignal.emit("view")

        elif item_id == "delete":
            self.deleteSignal.emit()

        elif item_id == "download":
