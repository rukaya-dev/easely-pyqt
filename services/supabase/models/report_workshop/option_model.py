...
logger = set_up_logger('main.services.supabase.models.report_workshop.option_model')
...
class OptionModel:
    @staticmethod
    async def get_options_paginate(search_text, filter_preferences, page_number, item_per_page):
        try:
            query = supabase.table('options').select("*", count='exact') \
                .order("option_id", desc=True)

            if search_text:
                query = query.or_(f"name.ilike.%{search_text}%,category_id.ilike.%{search_text}%")

            if filter_preferences:
                query = await OptionModel.apply_filter(filter_preferences)
    ...
    async def get_option_by_id(option_id):
        try:
            data = await supabase.table('options').select("*").eq('option_id', option_id) \
                .execute()
            if data.data:
                return data.data[0]
    ...
    async def create_option(data):
        try:
            data = await supabase.table('options').insert(data).execute()
            if data:
                return data.data
        except Exception as e:
            logger.error(e, exc_info=True)
    ...
    async def check_if_option_exist(name, category_id):
        try:
            data = await supabase.table('options').select('name').eq('name', name).eq("category_id",
                                                                                      category_id).execute()
            if len(data.data) > 0:
                return True
            return False
    ...
    async def delete_option(option_id):
        try:
            data = await supabase.table('options').delete() \
                .eq('option_id', option_id).execute()
            if data.data:
                return True
        except Exception as e:
            logger.error(e, exc_info=True)
    ...
    async def update_option(option_id, data):
        try:
            data = await supabase.table('options').update(data).eq('option_id', option_id).execute()
            if data:
                return data
        except Exception as e:
            logger.error(e, exc_info=True)
    ...
    async def check_if_updated_option_exist(option_id, name, category_id):
        try:
            data = await supabase.table('options').select('name') \
                .neq('option_id', option_id). \
                eq('name', name) \
    ...
    async def apply_filter(data):
        try:
            ...
            if data["option_name"].get("enabled"):
                query = query.eq('name', data["option_name"].get("option_name_value"))
            ...
            query = query.order("option_id", desc=True)
    ...
    async def get_options_by_category(category):
        try:
            data = await supabase.table('options').select("*").eq('category_id', category) \
                .execute()
            if data.data:
                return data.data
    ...
    async def get_options_by_slugs(options_slugs, category_id):
        try:
            ...
            for option_slug in options_slugs:
                data = await supabase.table('options').select("*").eq('slug', option_slug) \
                    .eq('category_id', category_id) \
                    .execute()

                if data.data:
                    options_data.append(data.data[0])
