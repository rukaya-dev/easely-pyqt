...
logger = set_up_logger('main.services.supabase.models.staff.referring_doctor.referring_doctor_model')
...
class ReferringDoctorModel:
    @staticmethod
    async def get_referring_doctors_paginate(search_data, filter_preferences, page_number,
                                      item_per_page):
        try:
            query = (supabase.table('referring_doctors').select(
                "*",
                count='exact')
                     .order("doctor_id", desc=True))

            if filter_preferences:
                query = await ReferringDoctorModel.apply_filter(filter_preferences)

            if search_data:
                first_name, last_name = extract_names(search_data)
                qr = f"first_name.ilike.%{first_name}%,category.ilike.%{search_data}%,specialty.ilike.%{search_data}%,phone_number.ilike.%{search_data}%,email.ilike.%{search_data}%"
                if last_name != "":
    ...
    @staticmethod
    async def get_referring_doctor_by_id(doctor_id):
        try:
            data = await supabase.table('referring_doctors').select(
                "*") \
    ...
    async def create_referring_doctor(data):
        try:
            data = await supabase.table('referring_doctors').insert(data).execute()
            if data.data:
                return data.data
    ...
    @staticmethod
    async def delete_referring_doctor(doctor_id):
        try:
            data = await supabase.table('referring_doctors').delete().eq('doctor_id', doctor_id).execute()
    ...
    @staticmethod
    async def update_referring_doctor(doctor_id, data):
        try:
            data = await supabase.table('referring_doctors').update(data).eq('doctor_id', doctor_id).execute()
            if data:
                return data
        except Exception as e:
            logger.error(e, exc_info=True)
